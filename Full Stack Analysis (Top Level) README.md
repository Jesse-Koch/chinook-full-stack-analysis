...
## Overview
A three-phase analytics pipeline built on the Chinook digital media store database, demonstrating how the same relational data and business questions are extracted, reported, and visualized using three different tools — mirroring a realistic analyst workflow rather than three disconnected single-tool exercises.
...
Rock generates more revenue ($826.65) than the next three genres combined, and dominates playlist curation at a comparable scale (3,238 appearances, more than every other genre combined). This consistency across two independent measures — purchasing and curation — is a stronger signal than either alone.
*Recommendation:* prioritize Rock/Metal titles in catalog acquisition and promotional placement, where demonstrated demand concentration is strongest.
...
*Recommendation:* investigate whether Steve Johnson's approach to customer relationships is replicable. If so, rebalancing a small number of accounts from Jane Peacock to Steve Johnson — or applying his approach across the team — could raise average customer value without requiring new customer acquisition.

**3. Organizational capacity**
5 of 8 employees show zero customer-linked activity, entirely concentrated in a single non-sales branch of the reporting hierarchy (under Michael Mitchell). Only the three employees reporting to Nancy Edwards handle customer accounts.
*Recommendation:* if customer-facing capacity ever needs to expand (for example, if any single rep's portfolio is judged too large), this analysis identifies exactly where in the org chart any cross-trained support would need to be sourced from.
...
- Open `Chinook_PowerBI_Dashboard.pbix` in Power BI Desktop for the full interactive experience, or view the included PDF export for a quick static preview
