## Chinook Full-Stack Analysis — Phase 1: SQL (MySQL)
...
2. Artist revenue shows a pronounced long tail: four of the top five revenue-generating artists (Iron Maiden, Metallica, Led Zeppelin, and others) are Rock/Metal acts, directly explaining Rock's genre-level dominance. Dozens of classical/orchestral artists generated only $0.99 each, from a single track sale.
3. The reporting hierarchy (via self-join) shows two branches under General Manager Andrew Adams: Nancy Edwards oversees the three employees who actually manage customer accounts, while Michael Mitchell's branch has no customer-facing responsibilities, explaining why 5 of 8 employees showed no customer-linked activity in the analysis.
4. Among active sales reps, total spend and per-customer value diverge: Jane Peacock generates the highest total spend ($833.04) and highest per-customer invoice by managing the most customers (21), but Steve Johnson's smaller portfolio (18 customers) generates the highest average spend per customer ($40.01).
...
## Files
chinook_sql_ queries.sql — all queries organized by business question
...
Query outputs from this analysis feed into the Excel stakeholder report (see /excel) and the Power BI dashboard (see /powerbi).
